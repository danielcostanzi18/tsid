...
class TsidManipulator:
    ...
    def __init__(self, conf, viewer=True):
        self.conf = conf
        self.robot = tsid.RobotWrapper(str(conf.urdf), [str(conf.path)], False)
        robot = self.robot
...
class TsidManipulatorForce:
    """Standard TSID formulation for a robot manipulator
    - end-effector FORCE task
    - Postural task
    - torque limits
    - pos/vel limits
    """
    ...
    def __init__(self, conf, viewer=True):
        ...
        mu = 100  # friction coefficient
        fMin = -1000.0  # minimum normal force
        ...
        self.eeForceTask = tsid.TaskContactForceEquality(
            "task-ee-force",
            robot,
            conf.dt,
            contact
